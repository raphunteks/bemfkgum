...
// Inisiasi Upstash Redis (Fallback jika kredensial belum ada)
const redis = new Redis({
  url: process.env.UPSTASH_REDIS_REST_URL || '[https://helped-monster-12345.upstash.io](https://helped-monster-12345.upstash.io)',
...
app.post('/api/plasma', async (req, res) => {
  try {
    ...
    // Simpan ke Redis (Fallback try-catch agar tidak crash jika Redis mati)
    try {
        await redis.hset('Aspirations', { [id]: JSON.stringify(payload) });
    } catch(err) {
        console.warn("Redis belum terkoneksi. Data dummmy disimulasikan.");
    }
